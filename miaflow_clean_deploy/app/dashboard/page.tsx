...
export default function DashboardPage() {
  ...
  const [recommendations, setRecommendations] = useState<Recommendation[]>(() => [
    ...runInventoryAgent(sampleSales),
    ...runMarketingAgent(sampleSales)
  ]);
  const [orders, setOrders] = useState<PhoneOrder[]>(sampleOrders);
  const [activity, setActivity] = useState<string[]>([
    "Demo data loaded",
    "Inventory Agent created initial recommendations",
    "Phone Order Agent created sample drafts"
  ]);
    ...
    <main className="dashboard-layout">
      <aside className="sidebar">
        ...
        {[
          ...
          <button key={id} onClick={() => setTab(id as Tab)} className={`side-btn ${tab === id ? "active" : ""}`}>
            {label}
          </button>
        ))}
      ...
      <section className="dash-main">
        ...
        {tab === "upload" && (
          <div className="card card-pad">
            ...
            <button className="btn btn-white" onClick={downloadTemplate} style={{ marginTop: 16 }}>Download template</button>
          </div>
        )}
          ...
          <div>
            ...
            <div className="grid-2" style={{ marginTop: 18 }}>
              {recommendations.map(r => (
                <RecCard key={r.id} rec={r} onApprove={() => updateRec(r.id, "Approved")} onReject={() => updateRec(r.id, "Rejected")} />
              ))}
            </div>
          </div>
        )}
        ...
        {tab === "orders" && (
          <div>
            <div className="card card-pad">
              ...
              <textarea value={orderText} onChange={e => setOrderText(e.target.value)} placeholder="Paste raw customer order request" />
              <button className="btn btn-primary" onClick={addOrder} style={{ marginTop: 12 }}>Run Phone Order Agent</button>
            </div>
            <div className="grid-2" style={{ marginTop: 18 }}>
              {orders.map(o => (
                <OrderCard key={o.id} order={o} onApprove={() => updateOrder(o.id, "Approved")} onReject={() => updateOrder(o.id, "Rejected")} />
              ))}
            </div>
          </div>
        )}
        ...
        <tbody>
          {products.map(p => (
            <tr key={p.product}>
              ...
              <td>
                <span className={p.trend === "up" ? "badge badge-ok" : p.trend === "down" ? "badge badge-warn" : "badge"}>
                  {p.trend}
                </span>
              </td>
            </tr>
...
function RecCard({ rec, onApprove, onReject }: { rec: Recommendation; onApprove: () => void; onReject: () => void }) {
  return (
    <div className="card card-pad">
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
        <span className={rec.priority === "High" ? "badge badge-red" : rec.priority === "Medium" ? "badge badge-warn" : "badge"}>
          {rec.priority}
        </span>
        <span className={rec.status === "Pending" ? "badge badge-warn" : rec.status === "Approved" ? "badge badge-ok" : "badge badge-red"}>
          {rec.status}
        </span>
      </div>
...
function OrderCard({ order, onApprove, onReject }: { order: PhoneOrder; onApprove: () => void; onReject: () => void }) {
  return (
    <div className="card card-pad">
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
        <span className="badge">Phone Order Draft</span>
        <span className={order.status === "Pending" ? "badge badge-warn" : order.status === "Approved" ? "badge badge-ok" : "badge badge-red"}>
          {order.status}
        </span>
      </div>
